    ...
    include_once $filePath;
      if($_SERVER['REQUEST_METHOD']=='GET'){
            ...
            if(isset($_POST['update'])){
                ...
                $sql="UPDATE training SET description='".$_POST['Description']."',skills='".$_POST['Skills']."' WHERE id='".$_POST['id']."'";
                if($conn->query($sql)){
              ...
              <form class="form col-8" action="" method="post" enctype="multipart/form-data">
                <div class="form-group  ">
                  ...
                  <div class="form-group  mb-2">
                   ...
                   <textarea class="form-control text-primary" rows="5"  name="Description"><?php echo $row['description']; ?></textarea>
                  </div>
                  <div class="form-group  ">
                      <label >Skills Earned</label>
                     <!--  <input class="form-control" type="date" placeholder="Date of Birth" name="birthdate" required> -->
                      <textarea class="form-control text-primary" rows="5"  name="Skills"><?php echo $row['skills']; ?></textarea>
                  </div>
                  <div class="form-group  ">
                      
                     <?php if(!empty($row['schoolremarks'])){
                      echo '<label class="text-white">School Supervisor Remarks</label><textarea readonly class="form-control text-primary" rows="5"  name="Skills">'.$row['skills'].'</textarea>';

                     } ?>
                  </div>
                  <div class="form-group  ">
                      
                     <?php if(!empty($row['industryremarks'])){
                      echo '<label class="text-white">School Supervisor Remarks</label><textarea  readonly class="form-control text-primary" rows="5"  name="Skills">'.$row['skills'].'</textarea>';
